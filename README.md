...
SECDatabase.com initially use the dataset published by SEC first but untunately it has many limitations. SECDatabase.com eventually decide to build a new, complete dataset in an easy to consume format. This dataset has numerious improvements over the dataset on SEC website in the aspects of data accuracy, coverage, and ease of data usage. Here are some examples:
...
4. Eliminated the needs to understand the convoluted XBRL convention as much as possible, so that Data Scientiest can focus on the data itself to derived the value.
